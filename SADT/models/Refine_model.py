...
class RefineModel(DistangleModel):
    ...
    def initialize(self, opt):
        ...
        self.netR = networks.define_D(512, opt.init_type, opt.init_gain, self.gpu_ids)
    ...
    def forward(self):
        inputG = torch.cat([self.input_img, self.shadow_color, self.shadow_mask], 1)
        #inputX = torch.cat([self.input_img, self.shadow_color, self.shadow_L], 1)
        inputM = self.netR(inputG)
        #print(inputM.shape)
        b, c, h, w = self.input_img.shape
        #self.input_img  
        #self.finali = inputM[:, 0:3, :, :] + self.input_img
        #print(self.finali.shape)
        self.final = inputM * self.shadow_mask_dilate + self.shadow_color * (1 - self.shadow_mask_dilate)
        #print(self.finalc.shape)
        #self.finall = inputM * self.shadow_mask_dilate
        #print(self.shadow_L.shape)
        #self.final = torch.cat([self.finali, self.finalc, self.finall], 1)

    def backward(self):
        # criterion = self.criterionL1
        lambda_ = self.opt.lambda_L1
        
        #print(self.shadowfree.shape)
        if self.tv_loss > 0:
        ...
        if self.pgrad_loss > 0:
            pgrad_loss = PoissonGradientLoss()(target=self.input_img, blend=self.final,
                                               source=self.shadowfree, mask=self.shadow_mask_dilate) \
                         * lambda_ * self.pgrad_loss
        else:
            pgrad_loss = 0.0

        if self.penum_loss > 0:
            penum_loss = PenumbraLoss()(E_mask = self.E_mask, target=self.shadowfree_color, source=self.final,
                                        erode=self.shadow_mask_erode, dilate=self.shadow_mask_dilate) \
                         * lambda_ * self.penum_loss
        else:
            penum_loss = 0.0

        self.loss_rescontruction = nn.L1Loss(reduction='sum')(self.final, self.shadowfree_color) * lambda_
