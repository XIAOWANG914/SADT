...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 ...
                 num_features = [96, 192, 384, 768], window_size=7, mlp_ratio=4., qkv_bias=True, qk_scale=None,
        ...
        for i_layer in range(self.num_layers):
            ...
            self.norm.append(norm_layer(self.num_features[i_layer]))
            self.reso.append(patches_resolution[0] // (2 ** i_layer))
    ...
    def forward_features(self, x):
        ...
        for i, layer in enumerate(self.layers):
            x, out = layer(x)
            out = self.norm[i](out)  # B L C
            B, L, C = out.shape
            out = out.view(B, self.reso[i], self.reso[i], C).permute(0, 3, 1, 2).contiguous()
            outs.append(out)
        return outs

    def forward(self, x):
        xx = self.forward_features(x)
        #print(xx[0].shape)
        xx = self.head(xx)
        #xx = xx+x[:,0:3,:,:]
        #print(x.shape)
