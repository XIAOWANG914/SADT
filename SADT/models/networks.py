...
def define_S(netG):
    ...
    if netG == 'large':
        ...
        #init_cfg=dict(type='Pretrained', checkpoint=checkpoint_file), init_cfg
        net = SwinTransformer(img_size, patch_size, in_chans, num_classes, embed_dim, depths, num_heads, num_features)
    elif netG == 'base':
        embed_dim=128
        depths=[2, 2, 18, 2]
        num_heads=[4, 8, 16, 32]
...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, gpu_ids=[]):
    ...
    if netG == 'resnet_9blocks':
        ...
        net = UnetGenerator2(input_nc, output_nc, 8, ngf, norm_layer=norm_layer, img_size=512, use_dropout=use_dropout)
    elif netG == 'unet_32':
...
def define_D(size = 512, init_type='normal', init_gain=0.02, gpu_ids=[]):
    net = None
    #from .my import resnext101_32x8d
    #net = resnext101_32x8d(pretrained=False, img_size=size)
    from .my import resnet34
    net = resnet34(pretrained=False, img_size=size)
    
    return init_net(net, init_type, init_gain, gpu_ids)

def define_X(size = 512, init_type='normal', init_gain=0.02, gpu_ids=[]):
    net = None
    #from .my import resnext101_32x8d
    #net = resnext101_32x8d(pretrained=False, img_size=size)
    from .my2 import resnet101
    net = resnet101(pretrained=False, img_size=size)
...
class UnetGenerator2(nn.Module):
    def __init__(self, input_nc, output_nc, num_downs, ngf=64,
                 norm_layer=nn.BatchNorm2d, img_size=512, use_dropout=False):
        ...
        self.downconv1 = nn.Conv2d(input_nc, ngf, kernel_size=3,
                             stride=1, padding=1, bias=use_bias)
        self.downrelu = nn.LeakyReLU(0.2, True)
        self.downconv2 = nn.Conv2d(ngf, ngf, kernel_size=3,
                             stride=1, padding=1, bias=use_bias)        
        self.downconv3 = nn.Conv2d(ngf, ngf, kernel_size=3,
                             stride=1, padding=1, bias=use_bias)
        self.uprelu = nn.ReLU(True)
        self.uprelu2 = nn.Tanh()
        self.upconv1 = nn.Conv2d(ngf * 2, output_nc, kernel_size=3, stride=1, padding=1)
#        self.upconv1 = nn.Conv2d(ngf * 2, ngf, kernel_size=3, stride=1, padding=1)
        self.upconv2 = nn.Conv2d(ngf, ngf, kernel_size=3, stride=1, padding=1)
        self.upconv3 = nn.Conv2d(ngf, output_nc, kernel_size=1)
        self.upnorm = norm_layer(ngf)
        
#        up1 = [self.upconv1, self.uprelu]
        up = [self.upconv3, nn.Tanh()]
        #up = [uprelu, upconv1, norm_layer(inner_nc), uprelu, upconv2, norm_layer(inner_nc), uprelu, upconv3, nn.Tanh()] # ]
        #nn.LeakyReLU(0.9, True)nn.ReLU(True)
        
        #self.swin1 = BasicLayer(dim=ngf, input_resolution=to_2tuple(img_size), depth = 2, num_heads=2)
        #self.swin4 = BasicLayer(dim=self.num_features[3], input_resolution=to_2tuple(img_size/8), depth = 2, num_heads=heads[3])

        self.up = nn.Sequential(*up)
        self.model = unet_block

    def forward(self, input):
        x = self.downconv1(input)
        #x = self.upnorm(x)
        #x = self.downrelu(x)
        ...
        v = self.uprelu(v)
        v = self.upconv1(v)
        #v = self.upconv1(torch.cat([xs, v], 1))
        #v = self.upnorm(v)
        #v = self.uprelu(v)
        #v = self.upconv2(v)
...
class UnetSkipConnectionBlock(nn.Module):
    def __init__(self, outer_nc, inner_nc, input_nc=None,
        ...
        if outermost:
            ...
            #                            padding=1)
            #upsam = Upsample(scale_factor=2, mode='bilinear')
            #upconv1 = nn.Conv2d(inner_nc * 2, inner_nc, kernel_size=3, stride=1, padding=1)
            #upconv2 = nn.Conv2d(inner_nc, inner_nc, kernel_size=3, stride=1, padding=1)
            #upconv3 = nn.Conv2d(inner_nc, outer_nc, kernel_size=1)
            upconv = nn.Conv2d(inner_nc * 2, outer_nc, kernel_size=3, stride=1, padding=1)
            down = [downconv1, downrelu]
            #up = [uprelu, upconv1, norm_layer(inner_nc), uprelu, upconv2, norm_layer(inner_nc), uprelu, upconv3] # , nn.Tanh()]
            up = [uprelu, upconv] 
...
class SwinBlock(nn.Module):
    ...
    def __init__(self, dim=96, input_resolution=[224,224], num_heads=3, window_size=7, shift_size=0,
