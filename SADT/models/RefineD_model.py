...
class RefineDModel(DistangleModel):
    def name(self):
    ...
    def initialize(self, opt):
        ...
        self.netD = networks.define_X(256, opt.init_type, opt.init_gain, self.gpu_ids)
        ...
        if self.isTrain:
            ...
            if opt.optimizer == 'adam':
                ...
                self.optimizer_D = torch.optim.SGD(self.netD.parameters(), momentum=0.9,
    ...
    def set_input(self, input):
        ...
        self.shadow_mask = torch.cat([input['C'],input['B']],1).to(self.device)
        #self.shadow_mask2 = input['C'].to(self.device)
    ...
    def backward(self):
        ...
        classweight=torch.tensor([1.0, 4.6]).to(self.device) 
        #print(self.final.shape)
        #print(self.shadow_mask.shape)
        self.loss_rescontruction = nn.CrossEntropyLoss(weight=classweight,reduction='sum')(self.final, self.shadow_mask) * lambda_
        self.loss = self.loss_rescontruction + tv_loss + grad_loss
        self.loss.backward()
    ...
    def vis(self, e, s, path='', eval=False):
        ...
        if not eval:
            ...
            img1 = output[0, 1,...]
            img = torch.unsqueeze(img1, dim=0)
            #img = torch.cat([img1, img1, img1], 0)
            #print(img.shape)
